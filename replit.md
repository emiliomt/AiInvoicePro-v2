...
- January 29, 2025. Enhanced PDF-only processing symmetry in Python RPA service: Added dedicated PDF directory (pdfPath) configuration with consistent directory management, updated all PDF processing methods to use dedicated self.pdf_dir instead of nested directories, implemented PDF table support in init_database method, ensured process_files_through_manual_pipeline properly routes PDF files through OCR extraction pipeline via /api/rpa/process-pdf endpoint, made PDF processing behavior fully symmetric with XML processing for robust end-to-end functionality.
- January 29, 2025. Added comprehensive debug logging for PDF login failures: Implemented detailed debug output throughout Python RPA service initialization, driver setup, and login processes to identify differences between XML and PDF import configurations, added file-type-specific debug captures including PDF_login_error, PDF_login_timeout_debug, and PDF_login_exception_debug for visual comparison, enhanced configuration logging with complete credential validation and path verification to diagnose why PDF imports fail login while XML imports succeed with identical credentials.
- January 29, 2025. Diagnosed and improved Chrome browser stability issues: Identified that both PDF and XML imports fail with identical browser crashes during ERP login process, indicating environmental stability issue rather than configuration-specific problem. Implemented enhanced browser stability flags including crash reporter disabling, logging reduction, accelerated rendering disabling, and threading optimizations. Added universal stability improvements for both file types, retry mechanisms with enhanced error handling, and comprehensive debug capture system to track browser behavior differences between configurations.
