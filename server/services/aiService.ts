...
/**
 * Classify if an invoice is petty cash based on amount threshold
 */
export async function classifyPettyCash(
  invoiceData: any, 
  threshold: number = 100000 // Default threshold in COP
): Promise<{ isPettyCash: boolean; confidence: number; reason: string }> {
  try {
    const amount = parseFloat(invoiceData.totalAmount || '0');
    
    if (isNaN(amount)) {
      return {
        isPettyCash: false,
        confidence: 0,
        reason: 'Unable to determine amount'
      };
    }
    ...
    if (amount <= threshold) {
      return {
        isPettyCash: true,
        confidence: 0.9,
        reason: `Amount ${amount} is below petty cash threshold ${threshold}`
      };
    }

    // Additional checks for petty cash indicators
    const vendorName = (invoiceData.vendorName || '').toLowerCase();
    const description = (invoiceData.extractedData?.descriptionSummary || '').toLowerCase();
    
    const pettyCashKeywords = [
      'petty cash', 'petty', 'cash', 'expense', 'reimbursement',
      'office supplies', 'coffee', 'lunch', 'transport', 'taxi',
      'parking', 'toll', 'gas', 'fuel', 'maintenance'
    ];

    const hasPettyCashKeywords = pettyCashKeywords.some(keyword => 
      vendorName.includes(keyword) || description.includes(keyword)
    );

    if (hasPettyCashKeywords && amount <= threshold * 1.5) {
      return {
        isPettyCash: true,
        confidence: 0.8,
        reason: `Petty cash keywords detected and amount ${amount} is within extended threshold`
      };
    }
    ...
    return {
      isPettyCash: false,
      confidence: 0.9,
      reason: `Amount ${amount} exceeds petty cash threshold ${threshold}`
    };

  } catch (error) {
    console.error('Error in petty cash classification:', error);
    return {
      isPettyCash: false,
      confidence: 0,
...
/**
 * Enhanced line item classification with confidence scoring
 */
  ...
  try {
    ...
    for (const item of lineItems) {
      ...
      const categoryKeywords = {
        'Office Supplies': ['paper', 'pen', 'pencil', 'notebook', 'folder', 'stapler', 'tape', 'marker'],
        'IT Equipment': ['computer', 'laptop', 'monitor', 'keyboard', 'mouse', 'printer', 'scanner', 'software'],
        'Furniture': ['chair', 'desk', 'table', 'cabinet', 'shelf', 'furniture', 'office'],
        'Services': ['consulting', 'service', 'maintenance', 'repair', 'installation', 'training'],
        'Travel': ['flight', 'hotel', 'transport', 'taxi', 'parking', 'toll', 'gas', 'fuel'],
        'Meals': ['lunch', 'dinner', 'breakfast', 'coffee', 'food', 'restaurant', 'catering'],
        'Marketing': ['advertising', 'marketing', 'promotion', 'brochure', 'flyer', 'social media'],
        'Utilities': ['electricity', 'water', 'internet', 'phone', 'utilities', 'bills'],
        'Other': []
      };
      ...
      // If no good match found, use AI for classification
      if (bestConfidence < 0.3) {
        try {
          const aiCategory = await classifyLineItemWithAI(item.description);
          bestCategory = aiCategory.category;
          bestConfidence = aiCategory.confidence;
          matchedKeywords = aiCategory.keywords || [];
        } catch (aiError) {
      ...
      classifications.push({
        ...
        suggestedCategory: bestConfidence < 0.5 ? 'Review Required' : undefined
      });
    }
    ...
    console.error('Error in line item classification:', error);
    return [];
  }
}
 ...
 */
async function classifyLineItemWithAI(description: string): Promise<{
  category: string;
  confidence: number;
  keywords: string[];
}> {
  try {
    const prompt = `Classify this line item description into one of these categories:
    - Office Supplies
    - IT Equipment  
    - Furniture
    - Services
    ...
    const response = await openai.chat.completions.create({
      model: "gpt-4o-mini",
      messages: [
        { role: "system", content: "Classify line items into business expense categories." },
