...
import { lineItems, lineItemClassifications, classificationKeywords } from '../../shared/schema';
...
export class ClassificationService {
  ...
  static async initializeDefaultKeywords(): Promise<void> {
    try {
      ...
      const existingKeywords = await db.select().from(classificationKeywords).limit(1);
    ...
    try {
      ...
      if (existingClassification.length > 0) {
        if (!existingClassification[0].isManualOverride) {
          ...
          console.log(`⚠️ Skipped update for line item ${lineItemId} - manually overridden`);
        }
      } else {
        ...
        console.log(`✅ Created new classification for line item ${lineItemId}`);
      }
