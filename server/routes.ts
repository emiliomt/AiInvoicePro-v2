...
import {
  predictInvoiceIssues,
  storePredictiveAlerts,
...
import {
  applyColombianRules,
  clearColombianInvoiceCache,
} from "./services/colombianInvoiceExtractor";
...
async function processInvoiceAsync(invoice: any, fileBuffer: Buffer) {
  try {
    console.log(
      `Starting OCR processing for invoice ${invoice.id} (${invoice.fileName})`,
    );
    ...
    console.log(
      `OCR completed for invoice ${invoice.id}, text length: ${ocrText.length}`,
    );
    ...
    const isXmlContent =
      ocrText.trim().startsWith("<?xml") &&
      (ocrText.includes("<Invoice") ||
        ocrText.includes("<CreditNote") ||
        ocrText.includes("<AttachedDocument"));

    if (isXmlContent) {
      console.log(
        `XML content detected for invoice ${invoice.id}, using XML parser instead of AI`,
      );
      ...
      try {
        ...
        console.error(
          `XML parsing failed for invoice ${invoice.id}, falling back to AI:`,
          xmlError,
        );
    ...
    try {
      ...
      const validationData = {
        ...
        totalAmount: cleanedData.totalAmount
          ? parseFloat(cleanedData.totalAmount.toString())
          : 0,
        taxAmount: cleanedData.taxAmount
          ? parseFloat(cleanedData.taxAmount.toString())
          : 0,
        extractedData: extractedData, // This includes buyerTaxId for NIT validation
      };

      const validationResult =
        await storage.validateInvoiceData(validationData);
      ...
      if (validationResult.violations.length > 0) {
        console.log(
          `❌ Validation violations for invoice ${invoice.id}:`,
          validationResult.violations,
        );
      }
      if (validationResult.warnings.length > 0) {
        console.log(
          `⚠️ Validation warnings for invoice ${invoice.id}:`,
          validationResult.warnings,
        );
      }
    } catch (validationError) {
      console.error(
        `❌ Validation failed for invoice ${invoice.id}:`,
        validationError,
      );
    ...
    try {
      ...
      console.error(
        `Failed to update invoice ${invoice.id} with error status:`,
        updateError,
      );
    }
  }
...
// Enhanced processing function for line item classification
  // FIXED: Place this OUTSIDE registerRoutes function at the top level
async function processInvoiceLineItems(invoices: any[], userId: string): Promise<void> {
  console.log(`🔄 Starting line item classification for ${invoices.length} invoices`);

  const processId = `process-${Date.now()}`;
  let processedCount = 0;
  const totalCount = invoices.length; // ✅ FIX: Properly calculate total count
  ...
  try {
    ...
    for (const invoice of invoices) {
      try {
        console.log(`📄 Processing invoice ${invoice.id} (${processedCount + 1}/${totalCount})`);

        // Update invoice status to processing classification
        await storage.updateInvoice(invoice.id, {
          status: "classifying_items",
        ...
        if (lineItems.length > 0) {
          ...
          // Classify each line item using ClassificationService
          await ClassificationService.classifyInvoiceLineItems(invoice.id, userId);

          console.log(`✅ Successfully classified ${lineItems.length} line items for invoice ${invoice.id}`);
        } else {
          console.log(`ℹ️ No line items found for invoice ${invoice.id}`);
        }

        // ✅ FIX: Update invoice status to 'classified' instead of 'extracted'
        await storage.updateInvoice(invoice.id, {
          status: "classified", // FIXED: was incorrectly "extracted"
          ...
          console.log(`✅ Updated invoice ${invoice.id} status to "classified" (${processedCount}/${totalCount})`);

        } catch (error) {
          console.error(`❌ Failed to process invoice ${invoice.id}:`, error);

          // Update invoice status to failed but continue processing others
          await storage.updateInvoice(invoice.id, {
            status: "processing_failed",
            processingError: error instanceof Error ? error.message : "Unknown error",
          });

          processedCount++; // Still increment to maintain progress
        }
      }

      console.log(`🎉 Line item processing completed: ${processedCount}/${totalCount} processed`);

    } catch (error) {
      console.error("❌ Fatal error in line item processing:", error);
      throw error;
    }
  }
  try {
    ...
    for (const invoice of validInvoices) {
      try {
        ...
        const currentPercentage = Math.round(
          (processedCount / totalInvoices) * 100,
        );
        console.log(
          `📈 Progress: ${processedCount}/${totalInvoices} (${currentPercentage}%)`,
        );
        ...
        if (lineItems.length === 0) {
          console.log(
            `⚠️ No line items found for invoice ${invoice.id}, skipping`,
          );
          processedCount++;
          continue;
        }
        ...
        const { ClassificationService } = await import(
          "./services/classificationService"
        );
        await ClassificationService.classifyInvoiceLineItems(
          invoice.id,
          userId,
        );

        // Verify results
        const finalLineItems = await storage.getLineItemsByInvoiceId(
          invoice.id,
        );
        const classifiedCount = await storage.getClassifiedLineItemCount(
          invoice.id,
        );

        const processingTime = Date.now() - startTime;
        console.log(
          `✅ Invoice ${invoice.id} completed in ${processingTime}ms:`,
        );
        console.log(
          `   📊 ${finalLineItems.length} unique line items, ${classifiedCount} classified`,
        );
    ...
    console.log(
      `   📊 Total: ${processedCount}, Success: ${successCount}, Failed: ${failedCount}`,
    );
    console.log(
      `   ✅ Success rate: ${Math.round((successCount / processedCount) * 100)}%`,
    );
  } catch (error) {
    console.error(
      `❌ Critical error in bulk classification session ${progressId}:`,
      error,
    );
    throw error;
  }
...
async function removeDuplicatesFromDatabase(
  ...
  try {
    ...
    const duplicateItems = allLineItems.filter(
    ...
    console.log(
      `🗑️ Removed ${duplicateItems.length} duplicate line items from database for invoice ${invoiceId}`,
    );
  } catch (error) {
    console.error(
      `❌ Error removing duplicates for invoice ${invoiceId}:`,
      error,
    );
  }
}
...
async function processInvoicesInBackground(
  sessionId: string,
  invoiceIds: number[],
  vendorContext: any,
) {
  ...
  try {
    ...
    const fetchedInvoices = await db
      .select()
      .from(invoices)
    ...
    fetchedInvoices.forEach((inv) => {
      console.log(
        `  - Invoice ${inv.id}: ${inv.invoiceNumber || "No number"} (${inv.vendorName || "No vendor"})`,
      );
    });

    if (fetchedInvoices.length === 0) {
      console.error(
        "ERROR: No invoices found in database for IDs:",
        invoiceIds,
      );
      progressTracker.updateSession(sessionId, {
        status: "error",
    ...
    console.log(
      `Loaded ${keywords.length} keyword categories for classification`,
    );
    ...
    for (let i = 0; i < fetchedInvoices.length; i++) {
      ...
      console.log(
        `Processing invoice ${i + 1}/${fetchedInvoices.length}: ${invoice.invoiceNumber || invoice.id}`,
      );
      ...
      try {
        ...
        successCount++;
        console.log(
          `Successfully processed invoice: ${invoice.invoiceNumber || invoice.id}`,
        );
      } catch (error) {
        console.error(
          `Failed to process invoice ${invoice.invoiceNumber || invoice.id}:`,
          error,
        );
        failedCount++;
      }
    ...
    console.log(
      `Invoice processing completed - Session: ${sessionId}. Processed: ${processedCount}, Success: ${successCount}, Failed: ${failedCount}`,
    );
  } catch (error) {
...
async function processInvoiceLineItems(
  ...
  console.log(
    `Processing line items for invoice: ${invoice.invoiceNumber || invoice.id}`,
  );
  ...
  try {
    ...
    console.log(
      `Found ${existingLineItems.length} existing line items in database`,
    );
    ...
    if (existingLineItems.length > 0) {
      ...
      console.log(
        `Creating ${invoice.extractedData.lineItems.length} line items from extracted data`,
      );
      ...
      const description =
        invoice.extractedData?.descriptionSummary ||
    ...
    console.log(
      `Processing ${itemsToClassify.length} line items for classification`,
    );
    ...
    for (let i = 0; i < itemsToClassify.length; i++) {
      ...
      console.log(
        `Classifying item ${i + 1}/${itemsToClassify.length}: "${item.description}"`,
      );
      ...
      try {
        ...
        if (existingClassification.length === 0) {
          ...
          await ClassificationService.classifyAndStore(
            item.id,
            "line-item-classifier",
          );
          classifiedItemsCount++;
          console.log(
            `Successfully classified item ${item.id}: "${item.description}"`,
          );
        } else {
          console.log(
            `Item ${item.id} already classified as: ${existingClassification[0].category}`,
          );
        }
      } catch (classifyError) {
    ...
    console.log(
      `✅ Line item processing completed for invoice ${invoice.invoiceNumber || invoice.id}: ${itemsToClassify.length} items processed, ${classifiedItemsCount} classified`,
    );
    ...
    console.log(
      `✅ Updated invoice ${invoice.id} status to "classified" after line item processing`,
    );
    ...
    progressTracker.updateSession(sessionId, {
      ...
      progress:
        70 +
        (totalLineItems > 0 ? (classifiedItemsCount / totalLineItems) * 20 : 0),
    });
  } catch (error) {
    console.error(
      `Error processing line items for invoice ${invoice.id}:`,
      error,
    );
    throw error;
  }
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Process line items for selected invoices endpoint
    // FIXED: Place this INSIDE registerRoutes function
    app.post('/api/process-invoices-line-items', isAuthenticated, async (req: any, res) => {
      try {
        ...
        console.log(`🚀 Starting line item classification for ${invoiceIds.length} invoices`);
        ...
        const invoices = await Promise.all(
          ...
          // ✅ Process invoices in background using the FIXED function
          setImmediate(async () => {
            try {
      ...
      // Filter invoices that can be processed (extracted status)
      const processableInvoices = invoices.filter(invoice => 
        invoice.status === 'extracted' // Only process extracted invoices
      );

      if (processableInvoices.length === 0) {
        return res.status(400).json({ 
          error: 'No invoices available for line item processing. Selected invoices may not be extracted yet.' 
        });
      }
  ...
  // Background processing function with fixes
  ...
  try {
    for (const invoice of invoices) {
      try {
        ...
        // Process line items classification
        const lineItems = await storage.getLineItemsByInvoiceId(invoice.id);
        ...
        if (lineItems.length > 0) {
          ...
          console.log(
            `✅ Successfully classified ${lineItems.length} line items for invoice ${invoice.id}`,
          );
        }
          ...
          // FIX: Send progress update with correct total count
            const { progressTracker } = await import('./services/progressTracker');
            progressTracker.updateProgress(userId, {
              processId,
              current: processedCount,
              total: totalCount, // FIX: Pass correct total count
              status: processedCount === totalCount ? "completed" : "processing",
              message: `Processed ${processedCount}/${totalCount} invoices`,
              timestamp: new Date(),
            });
          ...
          // Still update progress even on failure
            ...
            progressTracker.updateProgress(userId, {
              ...
              total: totalCount,
              status: "processing",
              message: `Processed ${processedCount}/${totalCount} invoices (1 failed)`,
              timestamp: new Date(),
            });
        }
      }
      ...
      // Final progress update
        ...
        progressTracker.updateProgress(userId, {
          ...
          total: totalCount,
          status: "completed",
          message: `Completed processing ${totalCount} invoices`,
          timestamp: new Date(),
        });

      console.log(
        `🎉 Line item processing completed for all ${totalCount} invoices`,
      );
    } catch (error) {
      console.error("❌ Fatal error in line item processing:", error);
  ...
  app.put("/api/petty-cash/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      res.status(500).json({
        message: "Failed to update petty cash log",
  ...
  app.post("/api/petty-cash/classify", isAuthenticated, async (req, res) => {
    try {
      const { invoiceId, isPettyCash, classificationMethod, confidenceScore } =
        req.body;

      if (!invoiceId || isPettyCash === undefined) {
        return res
          .status(400)
          .json({ message: "invoiceId and isPettyCash are required" });
      }
      ...
      if (existingLog) {
        ...
        console.log(
          `✅ Updated petty cash classification for invoice ${invoiceId}: ${isPettyCash ? "YES" : "NO"}`,
        );
        res.json({
        ...
        console.log(
          `✅ Created petty cash classification for invoice ${invoiceId}: ${isPettyCash ? "YES" : "NO"}`,
        );
        res.json({
      ...
      res
        .status(500)
        .json({ message: "Failed to store petty cash classification" });
    }
  });
  ...
  app.post("/api/petty-cash/recalculate", isAuthenticated, async (req, res) => {
    try {
      ...
      for (const invoice of invoices) {
        ...
        const existingLog = await storage.getPettyCashLogByInvoiceId(
          invoice.id,
        );
      ...
      console.log(
        `Recalculated petty cash: ${recalculatedCount} updated, ${newClassifications} new classifications`,
      );
      ...
      res
        .status(500)
        .json({ message: "Failed to recalculate petty cash classifications" });
    }
  });
  ...
  app.put("/api/settings/:key", isAuthenticated, async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json(
          setting || { key, value, message: "Setting updated successfully" },
        );
    } catch (error) {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes("foreign key constraint")) {
          return res.status(400).json({
            message:
              "Cannot delete project because it has associated records. Please remove dependencies first.",
          });
        }
      }
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes("foreign key constraint")) {
          return res.status(400).json({
            message:
              "Cannot delete projects because some have associated records. Please remove dependencies first.",
          });
        }
      }
    ...
    upload.any()(req, res, async (err) => {
      ...
      try {
        ...
        if (poFiles.length === 0) {
          return res
            .status(400)
            .json({ message: "No purchase order files found" });
        }
        ...
        for (let i = 0; i < poFiles.length; i++) {
          ...
          try {
            console.log(
              `Processing file ${i + 1}/${poFiles.length}: ${fileName}`,
            );

            // Extract OCR text
            const ocrText = await processInvoiceOCR(file.buffer, i);
            console.log(
              `OCR completed for PO ${fileName}, text length: ${ocrText.length}`,
            );
            ...
            if (
              ...
              ocrText.includes("Please try re-uploading")
            ) {
              errors.push({
                fileName,
            ...
            if (extractedData.projectId) {
              ...
              const fuzzyMatch = await findBestProjectMatch(
              ...
              if (fuzzyMatch) {
                ...
                console.log(
                  `Fuzzy matched project "${extractedData.projectId}" to "${fuzzyMatch}"`,
                );
              } else {
                console.log(
                  `No fuzzy match found for project "${extractedData.projectId}", setting to null`,
                );
            ...
            if (extractedData.issueDate) {
              try {
                ...
                console.log(
                  `Invalid issue date format: ${extractedData.issueDate}`,
                );
                issueDate = null;
              }
            }
            ...
            if (extractedData.expectedDeliveryDate) {
              try {
                expectedDeliveryDate = new Date(
                  extractedData.expectedDeliveryDate,
                );
                // Check if date is valid
                ...
                console.log(
                  `Invalid expected delivery date format: ${extractedData.expectedDeliveryDate}`,
                );
                expectedDeliveryDate = null;
              }
            }
            ...
            const existingPO = await storage.getPurchaseOrderByPoId(
              extractedData.poId || `PO-${Date.now()}-${i}`,
            );
            ...
            console.log(
              `Purchase order saved with ID: ${newPurchaseOrder.id} for file: ${fileName}`,
            );
            ...
            errors.push({
              ...
              details:
                "Please ensure the file is a valid PDF and try again. If the problem persists, try converting the file to a different format.",
            });
          }
        }
  ...
  app.post(
    "/api/invoices/:id/assign-project",
    isAuthenticated,
    async (req, res) => {
      try {
        const invoiceId = parseInt(req.params.id);
  ...
  app.get(
    "/api/invoices/:id/project-matches",
    isAuthenticated,
    async (req, res) => {
      try {
        const invoiceId = parseInt(req.params.id);
  ...
  app.post(
    "/api/invoices/:id/find-project-matches",
    isAuthenticated,
    async (req, res) => {
      try {
        const invoiceId = parseInt(req.params.id);
        ...
        const potentialMatches =
          await storage.findPotentialProjectMatches(invoice);
  ...
  app.post(
    "/api/invoices/:id/create-project-match",
    isAuthenticated,
    async (req, res) => {
      try {
        const invoiceId = parseInt(req.params.id);
        ...
        console.log(
          `✅ Created project match for invoice ${invoiceId} to project ${projectId} with score ${matchScore}`,
        );
        res.json(match);
      } catch (error) {
  ...
  // Process line items for selected invoices endpoint
  app.post(
    "/api/process-invoices-line-items",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = (req.user as any).claims.sub;
        ...
        if (
          ...
          return res.status(400).json({
            error:
              "Invalid request: invoiceIds array is required and must not be empty",
          });
        }

        console.log(
          `🚀 Starting line item classification for ${invoiceIds.length} invoices`,
        );
        ...
        // Filter invoices that can be processed (extracted status)
        const processableInvoices = invoices.filter(
          (invoice) => invoice.status === "extracted", // Only process extracted invoices
        );

        if (processableInvoices.length === 0) {
          return res.status(400).json({
            error:
              "No invoices available for line item processing. Selected invoices may not be extracted yet.",
          });
        }
  ...
  app.post("/api/project-matching", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!invoiceId || !projectName) {
        return res
          .status(400)
          .json({ message: "invoiceId and projectName are required" });
      }
      ...
      console.log(
        `✅ Stored project match for invoice ${invoiceId}: ${projectName}`,
      );
      res.json({
  ...
  app.post("/api/invoices/process", isAuthenticated, async (req, res) => {
    try {
      const { invoiceId, isPettyCash, projectMatch, validationStatus } =
        req.body;
      ...
      if (isPettyCash !== undefined) {
        try {
          const existingLog =
            await storage.getPettyCashLogByInvoiceId(invoiceId);

          if (existingLog) {
            results.pettyCashResult = await storage.updatePettyCashLog(
              existingLog.id,
              {
                isPettyCash,
          ...
          console.log(
            `✅ Stored petty cash classification for invoice ${invoiceId}: ${isPettyCash ? "YES" : "NO"}`,
          );
        } catch (error) {
          console.error(`Error storing petty cash classification:`, error);
        }
      ...
      if (projectMatch) {
        try {
          const matchData = {
            invoiceId,
            projectId:
              typeof projectMatch === "string"
                ? projectMatch
                : projectMatch.projectId,
            matchScore:
              typeof projectMatch === "object"
                ? projectMatch.matchScore || 85
                : 85,
            status: "auto" as any,
            matchDetails: {
              matchedProject:
          ...
          results.projectMatchResult =
            await storage.createInvoiceProjectMatch(matchData);
          console.log(
            `✅ Stored project match for invoice ${invoiceId}: ${typeof projectMatch === "string" ? projectMatch : projectMatch.projectName}`,
          );
        } catch (error) {
          console.error(`Error storing project match:`, error);
        }
      ...
      if (validationStatus) {
        try {
          const updateData: any = {
            ...
            processingStatus:
              validationStatus === "pending" ? "validated" : "processed",
          };

          await storage.updateInvoice(invoiceId, updateData);
          results.validationResult = { status: validationStatus };
          console.log(
            `✅ Updated validation status for invoice ${invoiceId}: ${validationStatus}`,
          );
        } catch (error) {
  ...
  app.post(
    "/api/invoices/:id/set-active-project-match",
    isAuthenticated,
    async (req, res) => {
      try {
        const invoiceId = parseInt(req.params.id);
  ...
  app.get(
    "/api/project-matches/unresolved",
    isAuthenticated,
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch unresolved project matches" });
      }
    },
  );
  ...
  app.get(
    "/api/predictive-alerts/:invoiceId",
    isAuthenticated,
    async (req, res) => {
      try {
        const invoiceId = parseInt(req.params.invoiceId);
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const userId =
          (req.user as any)?.claims?.sub || (req.user as any)?.id || "unknown";
    ...
    async (req, res) => {
      try {
        ...
        if (!setting) {
          ...
          const createPromise = storage.updateSetting(
            "user_preferences",
            defaultSettings.value,
          );
          const createTimeoutPromise = new Promise((_, reject) =>
            setTimeout(
              () => reject(new Error("Settings creation timeout")),
              10000,
            ),
          );
    ...
    async (req, res) => {
      try {
        ...
        if (value === undefined || value === null) {
          return res
            .status(400)
            .json({ message: "Settings value is required" });
        }
        ...
        try {
          ...
          if (existing?.value) {
            try {
              ...
              console.warn(
                "Failed to parse existing settings, using defaults:",
                jsonError,
              );
            }
          }
        ...
        if (typeof value === "object") {
          ...
          try {
            ...
            return res
              .status(400)
              .json({ message: "Invalid JSON format for settings value" });
          }
        } else {
    ...
    try {
      ...
      try {
        ...
        if (openaiKey) {
          ...
          console.log(
            "OpenAI key found, but using fallback suggestions for now",
          );
        }
      } catch (openaiError) {
      ...
      if (existing_keywords && existing_keywords.length > 0) {
        const existingSet = new Set(
          existing_keywords.map((k: string) => k.toLowerCase().trim()),
        );
        suggestions = suggestions.filter(
          (suggestion) => !existingSet.has(suggestion.toLowerCase().trim()),
        );
      }
      ...
      try {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          try {
            ...
            const vatReimbursement =
              row["VAT Reimbursement"] ||
              row["vatReimbursement"] ||
              row["VAT"] ||
              row["vat"];
            let vatNumber = "";
  ...
  app.get("/api/projects/template", isAuthenticated, async (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=project_validation_template.xlsx",
      );
      res.send(buffer);
    } catch (error) {
      ...
      try {
        ...
        const processPromises = invoiceFiles.map(async (file) => {
          try {
            ...
            console.log(
              `Created invoice record ${invoice.id} for file ${file.originalname}`,
            );
            ...
            setImmediate(async () => {
              try {
                console.log(
                  `Starting background processing for invoice ${invoice.id}`,
                );
                await processInvoiceAsync(invoice, file.buffer);
                console.log(
                  `Completed background processing for invoice ${invoice.id}`,
                );
              } catch (error) {
                console.error(
                ...
                try {
                  ...
                  console.error(
                    `Failed to update invoice ${invoice.id} with error:`,
                    updateError,
                  );
                }
              }
        ...
        console.log(
          `Successfully created ${uploadedInvoices.length} invoice records`,
        );
    ...
    async (req: any, res) => {
      try {
        ...
        const sanitizeText = (text: any) => {
          ...
          const specialCharRatio =
            (cleaned.match(/[^a-zA-Z0-9\s\-_.,]/g) || []).length /
            cleaned.length;
          if (specialCharRatio > 0.3 || cleaned.length > 500) {
            return null;
          }
    ...
    async (req: any, res) => {
      try {
        ...
        if (fs.existsSync(cacheDir)) {
          ...
          console.log(
            `Deleted ${deletedCacheCount} files from cache directory`,
          );
        }
  ...
  app.post("/api/invoices/:id/process", async (req: any, res) => {
    try {
      ...
      if (!invoice.fileUrl || !fs.existsSync(invoice.fileUrl)) {
        return res
          .status(400)
          .json({ message: "Invoice file not found on disk" });
      }
  ...
  app.post("/api/imported-invoices/process", async (req: any, res) => {
    try {
      ...
      console.log(
        `✅ Processing complete: ${result.processed} processed, ${result.failed} failed`,
      );
      ...
      res.status(500).json({
        ...
        console.log(
          `Starting invoice processing for line item classification - Session: ${progressId}`,
        );
        console.log(
          `📊 Progress session created: ${progressId} - Classification - ${invoiceIds.length} invoices`,
        );
        ...
        setImmediate(async () => {
          try {
            await processInvoiceLineItemsEnhanced(
              invoiceIds,
              progressId,
              user.claims.sub,
            );
            ...
            console.error(
              `❌ Line item processing failed for progress ID ${progressId}:`,
              error,
            );
          }
        });
  ...
  app.post("/api/imported-invoices/process/:logId", async (req: any, res) => {
    try {
      ...
      const result =
        await invoiceProcessingService.processInvoicesByLogId(logId);

      console.log(
        `✅ Processing complete for log ${logId}: ${result.processed} processed, ${result.failed} failed`,
      );
      ...
      console.error(
        `Error processing imported invoices for log ${req.params.logId}:`,
        error,
      );
      res.status(500).json({
        message: "Failed to process imported invoices",
      ...
      try {
        ...
        setImmediate(async () => {
          try {
            ...
            console.log(
              `Manual processing started for invoice ${invoiceId} (${invoice.fileName})`,
            );
            ...
            async function removeDuplicatesFromDatabase(
              ...
              try {
                const allLineItems =
                  await storage.getLineItemsByInvoiceId(invoiceId);
                const uniqueIds = uniqueLineItems.map((item) => item.id);
                const duplicateItems = allLineItems.filter(
                  (item) => !uniqueIds.includes(item.id),
                );
            ...
            console.error(
              `Manual processing failed for invoice ${invoiceId}:`,
              error,
            );
            await storage.updateInvoice(invoiceId, {
              status: "rejected",
        ...
        res
          .status(500)
          .json({ message: "Failed to start manual OCR processing" });
      }
    },
  );

  app.post(
    "/api/invoices/:id/extract-data",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const invoiceId = parseInt(req.params.id);
      ...
      try {
        ...
        const hasAccess =
          ...
          (invoice.userId === "rpa-system" &&
            user?.companyId === invoice.companyId);
        ...
        if (
          ...
          return res
            .status(400)
            .json({ message: "File not available for preview or not a PDF" });
        }
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        if (!hasAccess) {
          console.log(
            `Preview access denied for user ${userId} to invoice ${invoiceId}`,
          );
        ...
        if (fs.existsSync(filePath)) {
          ...
          doc
            .fontSize(12)
            .text(
              "This is a demonstration PDF generated for preview purposes.",
              100,
              260,
            );
          doc.text(
            "In production, this would be replaced with the actual uploaded PDF file.",
            100,
            280,
          );
          doc.text(
            "You can download this file using the download button.",
            100,
            300,
          );
          ...
          doc
            .fontSize(12)
            .text("This is page 2 of the demo invoice.", 100, 140);
  ...
  app.get("/api/invoices", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (includeMatches) {
        const invoicesWithMatches =
          await storage.getInvoicesWithProjectMatches(userId);
        ...
        const transformedInvoices = invoicesWithClassificationStatus.map(
          (invoice) => {
            ...
            if (invoice.invoiceNumber) {
              console.log(
                `Invoice ${invoice.invoiceNumber}: ${classificationCount} classifications out of ${totalLineItems} line items`,
              );
            }
            ...
            return {
              ...
              classificationStatus:
                classificationCount > 0 ? "classified" : "unclassified",
  ...
  app.delete(
    ...
    async (req: any, res) => {
      try {
        ...
        console.log(
          `Successfully deleted ${deletedCount} invoices for user ${userId}`,
        );
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const { AISuggestionService } = await import(
          "./services/aiSuggestionService"
        );
        const suggestions = AISuggestionService.analyzeExtractionErrors(
          invoice,
          invoice.ocrText || "",
        );
  ...
  async function applyColombianLearningUpdates(
    ...
    const corrections = [
      {
        ...
        correctedPattern:
          "Always include Colombian NIT check digit: XXXXXXXX-X format",
      ...
      },
      {
        ...
        correctedPattern:
          "Always include Colombian buyer NIT check digit: XXXXXXXX-X format",
      ...
      },
      {
        ...
        correctedPattern:
          'Extract "FECHA VENCIMIENTO" and convert DD/MM/YYYY to YYYY-MM-DD',
      ...
      },
      {
        ...
        correctedPattern:
          "Extract city from project delivery address, not vendor address",
        example:
          'From "Urbanización Parque Heredia... - Cartagena" → "Cartagena"',
      },
      {
        field: "vendorAddress",
        originalPattern: "Incorrectly assigning addresses",
        correctedPattern:
          "Colombian service invoices often have no vendor address - return null",
        example:
          "If no vendor address specified, return null (not project address)",
      },
      {
        field: "buyerAddress",
        ...
        example:
          'CONSTRUCCIONES OBYCON SAS address: "CL 93B 13 92 OF 303, Bogota D.C."',
      },
      {
        field: "vendorName",
        originalPattern: "Incomplete vendor name extraction",
        correctedPattern:
          "Extract complete vendor name including business type and description",
        example:
          'Full name: "ASOMA SEGURIDAD S.A. S. ASESORIA EN SALUD OCUPACIONAL, MEDIO AMBIENTE & SEGURIDAD"',
      },
    ];
    ...
    for (const correction of corrections) {
      if (correctedData[correction.field] !== undefined) {
        ...
        console.log(
          `🇨🇴 Stored Colombian learning insight for ${correction.field}:`,
    ...
    if (correctedData.vendorName?.includes("ASOMA")) {
      await storage.storeLearningInsight({
        ...
        suggestedFix:
          "ASOMA invoices: Extract full company name including S.A. and business description",
        frequency: 3,
        lastSeen: new Date(),
      });
    }
    ...
    if (correctedData.companyName?.includes("CONSTRUCCIONES")) {
      await storage.storeLearningInsight({
        ...
        suggestedFix:
          "CONSTRUCCIONES companies are typically buyers in Colombian construction invoices",
        frequency: 3,
        lastSeen: new Date(),
      });
    }
  ...
  async function storeColombianSpecificInsights(
    ...
    const insights = [
      ...
      },
      {
        ...
        value: JSON.stringify({
          pattern:
            "Colombian dates are DD/MM/YYYY format, convert to YYYY-MM-DD",
  ...
  async function writeEnhancedTrainingData(
    ...
    const trainingDataPath = path.join(
      process.cwd(),
      isColombianInvoice
        ? "colombian_training_feedback.jsonl"
    ...
    async (req: any, res) => {
      try {
        ...
        const isColombianInvoice = invoice.ocrText
          ? applyColombianRules(invoice.ocrText).isColombianInvoice
          : false;
        ...
        if (isColombianInvoice && correctedData) {
          await applyColombianLearningUpdates(
            invoice,
            correctedData,
            feedbackLog.id,
          );
        ...
        try {
          const { LearningTracker } = await import(
            "./services/learningTracker"
          );
        ...
        if (isColombianInvoice) {
          await storeColombianSpecificInsights(
            invoice.extractedData,
            correctedData,
          );
        }
        ...
        await writeEnhancedTrainingData(
          invoice,
          correctedData,
          isColombianInvoice,
          feedbackLog.id,
  ...
  app.post(
    "/api/invoices/:id/positive-feedback",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const invoiceId = parseInt(req.params.id);
        ...
        fs.appendFileSync(
          trainingDataPath,
          JSON.stringify(positiveTrainingEntry) + "\n",
        );

        res.json({
          message:
            "Thank you for the positive feedback! This helps us improve our AI extraction.",
          feedbackId: feedbackLog.id,
        });
      } catch (error) {
  ...
  app.get(
    "/api/erp/diagnostic/:connectionId",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const user = req.user;
        ...
        const hasAccess =
          connection.userId === (user as any).claims.sub ||
          (currentUser?.companyId &&
            connectionOwner?.companyId &&
            currentUser.companyId === connectionOwner.companyId);

        if (!hasAccess) {
          console.log(
            `Access denied: User ${(user as any).claims.sub} (company: ${currentUser?.companyId}) trying to access connection owned by ${connection.userId} (company: ${connectionOwner?.companyId})`,
          );
          return res
            .status(403)
        ...
        const diagnostics = {
          connectionTest:
            await erpAutomationService.testConnection(connectionData),
          timestamp: new Date().toISOString(),
          connectionInfo: {
            name: connection.name,
            baseUrl: connection.baseUrl,
  ...
  app.post("/api/validation-rules", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          message:
            "Missing required fields: name, fieldName, ruleType, ruleValue",
        });
      }
  ...
  app.get(
    "/api/validation-rules/validate-all",
    isAuthenticated,
    async (req: any, res) => {
      try {
  ...
  app.post(
    "/api/process-and-validate-pending",
    isAuthenticated,
    async (req, res) => {
      try {
        ...
        console.log(
          `Found ${pendingInvoices.length} pending invoices for validation`,
        );
        ...
        console.error(
          "Error processing and validating pending invoices:",
          error,
        );
        res
          .status(500)
          .json({ message: "Failed to process and validate pending invoices" });
      }
    },
  );
  ...
  app.post(
    "/api/process-approved-validations",
    isAuthenticated,
    async (req, res) => {
      try {
        ...
        for (const validation of validationResults.invoiceValidations) {
          if (validation.isValid) {
            ...
            const approvedProject = approvedProjects.find(
              (ap) => ap.invoiceId === validation.invoiceId,
            );
            ...
            if (approvedProject) {
              try {
                ...
                console.error(
                  `Error moving invoice ${validation.invoiceId} to verified:`,
                  error,
                );
              }
            }
        ...
        res
          .status(500)
          .json({ message: "Failed to process approved validations" });
      }
    },
  );
  ...
  app.get("/api/erp/connections", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const currentUser = await storage.getUser((user as any).claims.sub);
      let connections = await storage.getErpConnections(
      ...
      if (currentUser?.companyId) {
        ...
        for (const conn of allConnections) {
          ...
          if (
            connectionOwner?.companyId === currentUser.companyId &&
            conn.userId !== (user as any).claims.sub
          ) {
  ...
  app.post(
    "/api/invoice-importer/test-progress/:configId",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const configId = parseInt(req.params.configId);
        const { pythonInvoiceImporter } = await import(
          "./services/pythonInvoiceImporter"
        );
        ...
        res.json({
          message: `Test progress simulation started for config ${configId}`,
        });
      } catch (error) {
  ...
  app.get("/api/schedule-overview", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const currentUser = await storage.getUser((user as any).claims.sub);
      let configs = await storage.getInvoiceImporterConfigs(
        (user as any).claims.sub,
      );
      ...
      if (currentUser?.companyId) {
        ...
        for (const config of allConfigs) {
          ...
          if (
            configOwner?.companyId === currentUser.companyId &&
            config.userId !== (user as any).claims.sub
          ) {
      ...
      const scheduledConfigs = configs
        .filter(
          (config) =>
            config.scheduleType !== "once" && config.isActive === true,
        )
        .map((config) => {
          ...
          if (config.scheduleType === "daily") {
            ...
            const [hours, minutes] = (
              config.scheduleConfig?.timeOfDay ||
              config.scheduleTime ||
              "09:00"
            ).split(":");
  ...
  app.post(
    "/api/schedule-overview/:configId/toggle",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const configId = parseInt(req.params.configId);
    ...
    try {
      ...
      if (currentUser?.companyId) {
        ...
        filteredLogs = logs.filter(
          (log) => log.userId && companyUserIds.includes(log.userId),
        );
      } else {
        // If no company, only show user's own logs
        filteredLogs = logs.filter(
          (log) => log.userId === (user as any).claims.sub,
        );
      }
  ...
  app.put(
    ...
    async (req: any, res) => {
      try {
        ...
        const connection = await storage.updateErpConnection(
          connectionId,
          data,
        );
        ...
        if (updatedConnection) {
          console.log(`🔍 Verification - Updated connection ${connectionId}:`, {
            ...
            passwordLength: updatedConnection.password
              ? updatedConnection.password.length
              : 0,
          ...
          if (data.baseUrl || data.username || data.password) {
            console.log(
              `🔄 Syncing updated ERP credentials to import configurations using connection ${connectionId}`,
            );
            ...
            console.log(
              `✅ Import configurations synced with updated ERP connection credentials`,
            );
          }
        }
        ...
        console.error(
          `❌ Failed to update ERP connection ${req.params.id}:`,
          error,
        );
        const errorMessage =
          error instanceof Error ? error.message : "Unknown error";
  ...
  async function executeTaskAsync(
    ...
    const taskTimeout = setTimeout(
      async () => {
        console.log(
          `Task ${taskId} timed out after 20 minutes, marking as failed`,
        );
        await storage.updateErpTask(taskId, {
          status: "failed",
          errorMessage:
            "Task timed out after 20 minutes. Please try a simpler task or check the ERP system accessibility.",
        });
      },
      20 * 60 * 1000,
    );
    ...
    try {
      ...
      console.log(
        `🔐 Starting password decryption for connection: ${connection.name}`,
      );
      console.log(
        `🔐 Encrypted password (first 20 chars): ${connection.password ? connection.password.substring(0, 20) + "..." : "null/undefined"}`,
      );
      console.log(
        `🔐 Encrypted password length: ${connection.password ? connection.password.length : 0}`,
      );
      ...
      if (!base64Pattern.test(connection.password)) {
        console.error(
          `🔐 ERROR: Password does not appear to be valid base64: ${connection.password}`,
        );
      ...
      try {
        decryptedPassword = Buffer.from(connection.password, "base64").toString(
          "utf8",
        );
      } catch (decryptError) {
        console.error(`🔐 ERROR: Failed to decrypt password:`, decryptError);
        throw new Error(
          `Password decryption failed: ${decryptError instanceof Error ? decryptError.message : "Unknown decryption error"}`,
        );
      }
      ...
      if (!decryptedPassword || decryptedPassword.trim().length === 0) {
        ...
        throw new Error(
          `Password decryption failed: Empty password after decryption`,
        );
      }
      ...
      if (decryptedPassword === connection.password) {
        console.log(
          `🔐 WARNING: Decrypted password is identical to encrypted password - decryption may have failed`,
        );
      }
      ...
      const script = await erpAutomationService.generateRPAScript(
        taskDescription,
        connectionData,
      );
      ...
      const result = await erpAutomationService.executeRPAScript(
        script,
        connectionData,
        connection.userId,
        taskId,
  ...
  app.post("/api/workflows", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const workflow = await storage.createSavedWorkflow(
        data,
        (user as any).claims.sub,
      );
      res.json(workflow);
  ...
  app.get("/api/workflows", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const workflows = await storage.getSavedWorkflows(
        (user as any).claims.sub,
      );
      res.json(workflows);
    } catch (error) {
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const connection = await storage.getErpConnection(
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        if (
          !xmlSources ||
          !Array.isArray(xmlSources) ||
          xmlSources.length === 0
        ) {
          return res
            .status(400)
            .json({ error: "XML sources array is required" });
        }
        ...
        const result = await rpaService.batchProcessXMLInvoices(
          xmlSources,
          userId,
        );
    ...
    async (req: any, res) => {
      try {
        ...
        if (!data.connectionId) {
          return res.status(400).json({
            error:
              "ERP connection is required. Manual configurations are no longer supported for security reasons.",
          });
        }
        ...
        if (data.connectionId) {
          ...
          if (!connection) {
            return res.status(400).json({
              error:
                "ERP connection not found. Please select a valid ERP connection.",
            });
          }
          ...
          if (!hasAccess) {
            console.log(
              `Access denied: User ${(user as any).claims.sub} (company: ${currentUser?.companyId}) trying to use ERP connection owned by ${connection.userId} (company: ${connectionOwner?.companyId})`,
            );
            return res.status(400).json({
              error:
                "Invalid ERP connection. Please ensure you have selected a valid ERP connection that belongs to your account or company.",
            });
          }
          ...
          if (!connection.isActive) {
            return res.status(400).json({
              error:
                "Selected ERP connection is inactive. Please activate the connection before creating an import configuration.",
            });
          }

          console.log(
            `Creating import config using ERP connection: ${connection.name} (${connection.baseUrl}) - Company access: ${currentUser?.companyId === connectionOwner?.companyId}`,
          );
        }
        ...
        const config =
          await storage.createInvoiceImporterConfig(configWithUserId);
        res.json(config);
      } catch (error) {
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const configs = await storage.getInvoiceImporterConfigsByUser(
          (user as any).claims.sub,
        );
        res.json(configs);
      } catch (error) {
  ...
  app.put(
    "/api/invoice-importer/configs/:id",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const user = req.user;
        ...
        if (!config || config.userId !== (user as any).claims.sub) {
          return res
            .status(404)
            .json({ error: "Import configuration not found" });
        }

        // Parse and validate updates
        const updates = insertInvoiceImporterConfigSchema
          .partial()
          .parse(req.body);
        ...
        if (updates.isManualConfig === true) {
          return res.status(400).json({
            error:
              "Manual configurations are no longer supported for security reasons. Please use an ERP connection.",
          });
        }

        const updatedConfig = await storage.updateInvoiceImporterConfig(
          configId,
          updates,
        );
    ...
    async (req: any, res) => {
      try {
        ...
        if (config.userId !== (user as any).claims.sub) {
          return res
            .status(403)
            .json({ error: "Access denied to this import configuration" });
        }
  ...
  app.post(
    "/api/invoice-importer/configs/:id/execute",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const user = req.user;
        ...
        if (
          config.userId !== (user as any).claims.sub &&
          (!currentUser?.companyId ||
            config.companyId !== currentUser.companyId)
        ) {
          return res
            .status(403)
            .json({ error: "Access denied to this import configuration" });
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        console.log(
          `Starting import process for config ${configId}, log ID: ${log.id}`,
        );

        // Start the import process asynchronously but don't wait for it
        setImmediate(() => {
          pythonInvoiceImporter
            .executeImportTaskWithLogId(configId, log.id)
            .then(() => {
  ...
  app.get(
    "/api/invoice-importer/logs/:configId",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const user = req.user;
  ...
  app.get(
    "/api/invoice-importer/logs/config/:configId",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const user = req.user;
  ...
  app.get(
    "/api/invoice-importer/logs/:logId/invoices",
    isAuthenticated,
    async (req, res) => {
      try {
        const logId = parseInt(req.params.logId);
  ...
  app.post(
    "/api/invoice-importer/logs/:logId/process",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const logId = parseInt(req.params.logId);
        ...
        console.log(
          `🔄 Manually processing imported invoices for log ${logId}`,
        );
      ...
      try {
        ...
        for (const source of xmlSources) {
          if (!source.id || !source.content) {
            return res
              .status(400)
              .json({ error: "Each XML source must have id and content" });
          }
        }

        console.log(
          `Starting XML batch import for user ${userId}, ${xmlSources.length} sources`,
        );
  ...
  app.post("/api/rpa/process-pdf", async (req: any, res) => {
    try {
      ...
      console.log(
        `🔄 PRIORITY EXTRACTION: Processing RPA PDF file: ${filename} (config: ${configId})`,
      );
      ...
      if (configId) {
        try {
          ...
          if (config) {
            ...
            console.log(
              `📋 Retrieved company ID ${companyId} from config ${configId}`,
            );
          } else {
          ...
          console.warn(
            `❌ Could not retrieve config ${configId} for company ID:`,
            error,
          );
        }
      } else {
      ...
      const existingInvoices =
        await storage.getInvoicesByFileName(baseFileName);
      if (existingInvoices.length > 0) {
        console.log(
          `⚠️ Invoice with base name '${baseFileName}' already exists (${existingInvoices[0].id}), skipping PDF processing`,
        );
        return res.json({
          success: true,
      ...
      console.log(
        `Created invoice record ${invoice.id} for RPA PDF file ${filename}`,
      );
      ...
      setImmediate(async () => {
        try {
          await processInvoiceAsync(invoice, fileBuffer);
          console.log(
            `✅ RPA PDF invoice ${invoice.id} processed successfully`,
          );
        } catch (error) {
          console.error(
            `❌ RPA PDF invoice ${invoice.id} processing failed:`,
            error,
          );
        }
      });
  ...
  app.post("/api/invoice-importer/progress-update", async (req: any, res) => {
    try {
      ...
      if (latestLog) {
        ...
        console.log(
          `📊 Progress update: Config ${configId} - Processed: ${processedInvoices}, Success: ${successfulImports}, Failed: ${failedImports}`,
        );
        ...
        res
          .status(404)
          .json({ error: "No active import log found for this configuration" });
      }
    } catch (error) {
  ...
  app.get("/api/ai/colombian-insights", isAuthenticated, async (req, res) => {
    try {
      ...
      res.json({
        colombianInsights,
        generalInsights: generalInsights.filter(
          (insight) =>
            insight.field &&
            [
              "taxId",
  ...
  app.post(
    "/api/invoices/:id/reextract-colombian",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const invoiceId = parseInt(req.params.id);
        ...
        if (!invoice.ocrText) {
          return res
            .status(400)
            .json({ message: "No OCR text available for re-extraction" });
        }
        ...
        const newExtractedData = await extractInvoiceData(
          invoice.ocrText,
          true,
        );
        ...
        await storage.updateInvoice(invoice.id, {
          ...
          invoiceDate: newExtractedData.invoiceDate
            ? new Date(newExtractedData.invoiceDate)
            : null,
          dueDate: newExtractedData.dueDate
            ? new Date(newExtractedData.dueDate)
            : null,
          ...
          confidenceScore: parseFloat(
            newExtractedData.confidenceScore || "0.8",
          ),
        });

        res.json({
          message:
            "🇨🇴 Colombian invoice re-extracted successfully with enhanced rules",
          extractedData: newExtractedData,
          improvements: [
    ...
    async (req: any, res: any) => {
      try {
        ...
        `;
          ...
          console.log(
            `🔗 Found ${linkedPDFs.rows.length} linked PDF files for invoice ${invoiceId}`,
          );
          ...
          if (!fs.existsSync(pdfFile.file_path)) {
            ...
            console.log(
              ...
              fs.readdirSync("uploads").filter((f) => f.includes("FEPG793514")),
            );
            return res.status(404).json({
              ...
              troubleshooting:
                "The PDF file may not have been properly saved during RPA import",
            });
          }
          ...
          res.setHeader(
            "Content-Disposition",
            `inline; filename="${pdfFile.original_file_name}"`,
          );
  ...
  app.get(
    ...
    async (req: any, res: any) => {
      try {
        ...
        const userId = (req.user as any).claims.sub;
        console.log(
          `Download request for invoice ${invoiceId} by user ${userId}`,
        );
        ...
        console.log(
          `Found invoice: ${invoice.fileName}, owner: ${invoice.userId}, company: ${invoice.companyId}`,
        );
        ...
        if (!hasAccess) {
          console.log(
            `Access denied for user ${userId} to invoice ${invoiceId}`,
          );
          return res.status(403).json({ error: "Access denied" });
        }
        ...
        if (invoice.userId === "rpa-system") {
          console.log(
            `🔍 Checking for linked files for RPA invoice: ${invoice.fileName}`,
          );
          ...
          `;
            ...
            const linkedPDFs = await dbClient.query(linkedPDFQuery, [
              invoiceId,
            ]);
            console.log(
              `🔗 Found ${linkedPDFs.rows.length} linked PDF files for invoice ${invoiceId}`,
            ...
            if (linkedPDFs.rows.length >> 0) {
              ...
              for (const linkedPDF of linkedPDFs.rows) {
                ...
                if (fs.existsSync(pdfPath)) {
                  ...
                  linkedFilesAdded++;
                  console.log(
                    `📎 Added linked PDF to ZIP: ${linkedPDF.original_file_name}`,
                  );
                } else {
              ...
              console.log(
                `📦 ZIP package ready with 1 main file + ${linkedFilesAdded} linked PDFs`,
              );
              archive.finalize();
              return;
            }
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const processableInvoices = allInvoices.filter(
          (invoice) =>
        ...
        setImmediate(async () => {
          for (const invoice of processableInvoices) {
            try {
              if (
                invoice.fileUrl &&
                require("fs").existsSync(invoice.fileUrl)
              ) {
                const fs = require("fs");
                const fileBuffer = fs.readFileSync(invoice.fileUrl);
                await processInvoiceAsync(invoice, fileBuffer);
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const { AILineItemClassifier } = await import(
          "./services/aiLineItemClassifier"
        );
        ...
        try {
          ...
          console.log(
            "OpenAI API key not found, using keyword-based classification",
          );
        }
        ...
        const result = await classifier.classifyLineItem(
          lineItem,
          vendorContext,
        );
        res.json(result);
    ...
    async (req: any, res) => {
      try {
        ...
        if (!Array.isArray(lineItems) || lineItems.length === 0) {
          return res
            .status(400)
            .json({ error: "Line items array is required" });
        }
    ...
    async (req: any, res) => {
      try {
        ...
        const categories = {
          ...
          equipment_tools:
            "Tools, machinery, equipment, and hardware for operations",
          services_labor:
            "Professional services, labor, consulting, and expertise",
          utilities_facilities:
            "Utilities, facility costs, and operational overhead",
          food_beverages: "Food, beverages, and related consumables",
          transportation_logistics:
            "Transportation, shipping, logistics, and related services",
          technology_software:
            "Technology, software, digital services, and IT solutions",
          marketing_advertising:
            "Marketing, advertising, promotional materials and services",
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const testItems = [
          {
            description: "C S IND MUROPLACA 4",
            rawText: "C S IND MUROPLACA 4",
          },
