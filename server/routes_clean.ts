...
import { AILineItemClassifier } from './services/aiLineItemClassifier.js';
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  async function linkPDFsToExistingInvoices(configId: number) {
    try {
      ...
      const pdfFiles = files.filter(file => file.toLowerCase().endsWith('.pdf') || file.toLowerCase().endsWith('.zip'));

      console.log(`🔗 Found ${pdfFiles.length} PDF/ZIP files in uploads directory`);

      // Get the actual log ID from the current import session
      let actualLogId = 1; // fallback
      try {
        ...
        if (logs.length > 0) {
          ...
          console.log(`📝 Using log ID ${actualLogId} for PDF linking`);
        } else {
          console.warn(`⚠️ No logs found for config ${configId}, using fallback log ID ${actualLogId}`);
        }
      } catch (logError) {
        console.error('Could not fetch log ID, using fallback:', logError);
      }

      // Get all imported files for this specific log ID
      const importedFiles = await storage.getImportedInvoicesByLog(actualLogId);
      console.log(`📁 Found ${importedFiles.length} imported files for log ${actualLogId}`);

      // Get recently created invoices from RPA system
      const recentInvoicesQuery = `
        SELECT id, file_name, invoice_number, vendor_name, total_amount, created_at 
        FROM invoices 
      ...
      const invoicesResult = await dbClient.query(recentInvoicesQuery);
      const invoices = invoicesResult.rows;
      console.log(`📄 Found ${invoices.length} recent RPA invoices for linking`);

      let linkedCount = 0;

      // Separate PDF files from imported files that are not yet linked
      const pdfFilesToLink = importedFiles.filter(f => f.file_type === 'pdf' && !f.linked_invoice_id);
      console.log(`📁 PDFs to link: ${pdfFilesToLink.length}`);

      // Link PDFs to invoices based on creation order and file tokens
      for (let i = 0; i < Math.min(pdfFilesToLink.length, invoices.length); i++) {
        try {
          const pdfFileRecord = pdfFilesToLink[i];
          const invoice = invoices[i];

          // Check if the PDF file actually exists on disk
          const pdfFilePath = path.join(uploadDir, pdfFileRecord.original_file_name);
          if (!fs.existsSync(pdfFilePath)) {
            console.warn(`⚠️ PDF file not found on disk: ${pdfFilePath}`);
            continue;
          }

          // Update the imported_invoices record to link the PDF
          const updateQuery = `
            UPDATE imported_invoices 
            SET linked_invoice_id = $1,
          ...
          const metadata = {
            ...
            linkingMethod: 'automatic_rpa_enhancement',
            configId: configId,
          ...
          linkedCount++;
          console.log(`🔗 Enhanced linking: PDF ${pdfFileRecord.original_file_name} → Invoice ${invoice.id} (${invoice.invoice_number})`);

        } catch (error) {
          console.error(`❌ Error linking PDF file:`, error);
        }
      }

      await dbClient.end();
      console.log(`✅ Enhanced PDF linking completed: ${linkedCount} PDFs linked to invoices`);

    } catch (error) {
      console.error('❌ Error in enhanced automatic PDF linking:', error);
      throw error;
    }
  }
  ...
  app.get('/api/invoices/:id/linked-files', isAuthenticated, async (req: any, res) => {
    try {
      ...
      try {
        ...
        // Look for linked files in imported_invoices table
        const linkedFilesQuery = `
          SELECT * FROM imported_invoices 
          WHERE linked_invoice_id = $1 
          ORDER BY file_type DESC, created_at ASC
        `;

        const linkedResult = await dbClient.query(linkedFilesQuery, [invoiceId]);
        let linkedFiles = linkedResult.rows;

        // If no linked files found for RPA invoices, try to find by file matching
        if (linkedFiles.length === 0 && invoice.user_id === 'rpa-system') {
          console.log(`🔍 No direct links found for RPA invoice ${invoiceId}, searching by file patterns...`);

          // Try to find files that might be related based on invoice number or vendor
          const patternQuery = `
            SELECT * FROM imported_invoices 
            WHERE (
              original_file_name ILIKE $1 OR 
              original_file_name ILIKE $2 OR
              metadata->>'invoiceNumber' = $3
            )
          ...
          const vendorName = (invoice.vendor_name || '').split(' ')[0]; // First word of vendor

          const patternResult = await dbClient.query(patternQuery, [
            `%${invoiceNumber}%`,
            `%${vendorName}%`,
            invoiceNumber
          ]);
          ...
          if (patternResult.rows.length > 0) {
            console.log(`📎 Found ${patternResult.rows.length} potential matching files for invoice ${invoiceId}`);

            // Auto-link the first matching PDF file
            const pdfFile = patternResult.rows.find(f => f.file_type === 'pdf');
            if (pdfFile) {
              const linkQuery = `
                UPDATE imported_invoices 
                SET linked_invoice_id = $1,
              ...
              await dbClient.query(linkQuery, [
                invoiceId,
                JSON.stringify({
                  linkedAt: new Date().toISOString(),
                  linkingMethod: 'pattern_matching',
                  autoLinked: true
                }),
                pdfFile.id
              ]);
              ...
              linkedFiles = [pdfFile];
              console.log(`🔗 Auto-linked PDF ${pdfFile.original_file_name} to invoice ${invoiceId}`);
            }
          }
        }

        console.log(`🔍 Final result: ${linkedFiles.length} linked files for invoice ${invoiceId}`);
        ...
        const response = {
          ...
          linkedFiles: linkedFiles.map(file => ({
            ...
            isDataSource: file.is_data_source,
      ...
      console.error('Error getting linked files:', error);
...
Add endpoint to manually trigger PDF linking for RPA invoices
Replacing
<old_str>
// Manual RPA execution endpoint
  app.post('/api/invoice-importer/run/:configId', isAuthenticated, async (req: any, res) => {
</old_str>
with
<new_str>
// Manual PDF linking trigger for RPA imports
  app.post('/api/invoice-importer/link-pdfs/:configId', isAuthenticated, async (req: any, res) => {
    try {
      const configId = parseInt(req.params.configId);
      if (isNaN(configId)) {
        return res.status(400).json({ error: 'Invalid configuration ID' });
      ...
      console.log(`🔗 Manual PDF linking triggered for config ${configId}`);
      await linkPdfsToInvoices(configId);

      res.json({ 
        success: true, 
        message: 'PDF linking completed successfully' 
      });

    } catch (error) {
      console.error('Manual PDF linking failed:', error);
      res.status(500).json({ 
        error: 'Failed to link PDFs', 
...
This change improves the PDF linking logic for RPA imports by refining how PDF files are identified and associated with invoices, enhancing the accuracy and reliability of the linking process.
<replit_final_file>
import express, { type Express, type Request, type Response, type NextFunction } from "express";
import { createServer, type Server } from "http";
import multer from "multer";
    ...
    for (const line of lines) {
      ...
      if (quantityMatch && priceMatch) {
        ...
        currentItem = {
          description: trimmedLine,
          quantity: parseFloat(quantityMatch[1]),
          unit: quantityMatch.length > 2 ? quantityMatch[2] : undefined,
          unitPrice: parseFloat(priceMatch[1].replace(/,/g, ''))
        };
      } else if (trimmedLine.length > 3 && !trimmedLine.match(/^[\d\s\$,\.]+$/)) {
        // This line might be a description
    ...
    try {
      ...
      for (const pdfFile of pdfFiles) {
        try {
          ...
          const baseFileName = path.parse(pdfFile).name.split('_').slice(0, 2).join('_');
