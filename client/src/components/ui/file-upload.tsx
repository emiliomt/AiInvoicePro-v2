    ...
    (acceptedFiles: File[]) => {
      ...
      const validFiles = acceptedFiles.filter((file) => {
        if (file.size > maxSize) {
          console.warn(
            `FileUpload: File ${file.name} is too large (${file.size} bytes, max: ${maxSize} bytes)`,
          );
          return false;
        }
        return true;
  ...
  const handleClick = useCallback(() => {
    ...
    input.accept = Object.entries(accept)
      .map(([mimeType, extensions]) =>
        extensions.map((ext) => mimeType + ext).join(","),
      )
      .join(",");
    ...
    input.onchange = (e) => {
      ...
      if (target.files) {
        ...
        console.log(
          "FileUpload: Files selected via click:",
  ...
  const { getRootProps, getInputProps, isDragActive, isDragReject } =
    useDropzone({
      ...
      onDropAccepted: (files) => {
        console.log(
          "FileUpload: Dropzone accepted files:",
          files.map((f) => ({ name: f.name, size: f.size, type: f.type })),
        );
      },
      onFileDialogCancel: () => {
