      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Tabs defaultValue="keywords" className="space-y-6">
          <TabsList>
            ...
            <TabsTrigger value="visualize">AI Classification Analytics</TabsTrigger>
          </TabsList>
          ...
          {/* AI Classification Analytics Tab */}
          <TabsContent value="visualize">
            <div className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {lineItemClassifications.length}
                    </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-blue-600">
                      {lineItemClassifications.filter(item => item.matchedKeyword === 'AI Classification').length}
                    </div>
                    <p className="text-xs text-muted-foreground">
                      AI-powered results
                    </p>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-orange-600">
                      {lineItemClassifications.filter(item => item.isManualOverride).length}
                    </div>
                    <p className="text-xs text-muted-foreground">
                      Human corrections
                    </p>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-green-600">
                      {lineItemClassifications.length > 0 ? 
                        Math.round(lineItemClassifications
                          .filter(item => item.confidence)
                          .reduce((sum, item) => sum + parseFloat(item.confidence), 0) / 
                        lineItemClassifications.filter(item => item.confidence).length * 100) 
                        : 0}%
                    </div>
                    <p className="text-xs text-muted-foreground">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {Object.entries(CATEGORY_INFO).map(([key, info]) => {
                        const count = lineItemClassifications.filter(item => item.category === key).length;
                        const percentage = lineItemClassifications.length > 0 ? 
                          Math.round((count / lineItemClassifications.length) * 100) : 0;
                        const Icon = info.icon;
                          ...
                          <div key={key} className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              <div className="w-20 bg-gray-200 rounded-full h-2">
                                <div 
                                  className={`h-2 rounded-full ${info.color.includes('blue') ? 'bg-blue-500' : 
                                    info.color.includes('green') ? 'bg-green-500' :
                                    info.color.includes('purple') ? 'bg-purple-500' : 'bg-orange-500'}`}
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {[
                        ...
                        { label: 'Low (40-59%)', min: 0.4, color: 'bg-orange-500' },
                        { label: 'Very Low (0-39%)', min: 0, color: 'bg-red-500' }
                      ].map(range => {
                        const count = lineItemClassifications.filter(item => {
                          if (!item.confidence) return false;
                          const conf = parseFloat(item.confidence);
                          if (range.min === 0.8) return conf >= 0.8;
                          if (range.min === 0.6) return conf >= 0.6 && conf < 0.8;
                          if (range.min === 0.4) return conf >= 0.4 && conf < 0.6;
                          return conf < 0.4;
                        }).length;
                        const total = lineItemClassifications.filter(item => item.confidence).length;
              ...
              {/* AI vs Manual Classification Comparison */}
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <Bot className="w-5 h-5" />
                    AI vs Manual Classification Analysis
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <h4 className="font-medium mb-3">Classification Methods</h4>
                      <div className="space-y-3">
                        {[
                          { 
                            label: 'AI Classifications', 
                            count: lineItemClassifications.filter(item => item.matchedKeyword === 'AI Classification').length,
                            color: 'bg-blue-500',
                            icon: Bot
                          },
                          { 
                            label: 'Keyword Matching', 
                            count: lineItemClassifications.filter(item => item.matchedKeyword && item.matchedKeyword !== 'AI Classification' && item.matchedKeyword !== 'manual override').length,
                            color: 'bg-purple-500',
                            icon: Tag
                          },
                          { 
                            label: 'Manual Override', 
                            count: lineItemClassifications.filter(item => item.isManualOverride).length,
                            color: 'bg-orange-500',
                            icon: Users
                          }
                        ].map(method => {
                          const percentage = lineItemClassifications.length > 0 ? 
                            Math.round((method.count / lineItemClassifications.length) * 100) : 0;
                          const Icon = method.icon;
                          
                          return (
                            <div key={method.label} className="flex items-center justify-between p-3 border rounded-lg">
                              <div className="flex items-center gap-3">
                                <Icon className="w-4 h-4" />
                    ...
                    <div>
                      <h4 className="font-medium mb-3">AI Performance Insights</h4>
                      <div className="space-y-3">
                        <div className="p-3 border rounded-lg bg-blue-50">
                          <div className="flex items-center gap-2 mb-1">
                            <Check className="w-4 h-4 text-green-500" />
                            <span className="text-sm font-medium">High Confidence AI</span>
                          </div>
                          <div className="text-xs text-gray-600">
                            {lineItemClassifications.filter(item => 
                              item.matchedKeyword === 'AI Classification' && 
                              item.confidence && parseFloat(item.confidence) >= 0.8
                            ).length} classifications with 80%+ confidence
                          </div>
                        </div>
                        ...
                        <div className="p-3 border rounded-lg bg-orange-50">
                          ...
                          <div className="text-xs text-gray-600">
                            {lineItemClassifications.filter(item => 
                              item.matchedKeyword === 'AI Classification' && 
                              item.confidence && parseFloat(item.confidence) < 0.6
                            ).length} AI classifications with low confidence
                          </div>
                        </div>
                        ...
                        <div className="p-3 border rounded-lg bg-green-50">
                          ...
                          <div className="text-xs text-gray-600">
                            {lineItemClassifications.filter(item => item.isManualOverride).length} manual corrections to improve AI training
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <Card>
                ...
                <CardContent>
                  <Table>
                    ...
                    <TableBody>
                      {lineItemClassifications.slice(0, 10).map((item: any) => (
                        <TableRow key={item.lineItemId}>
                                ...
                                <div className="w-16 bg-gray-200 rounded-full h-2">
                                  <div 
                                    className={`h-2 rounded-full ${
                                      parseFloat(item.confidence) >= 0.8 ? 'bg-green-500' :
                                      parseFloat(item.confidence) >= 0.6 ? 'bg-yellow-500' :
                                      parseFloat(item.confidence) >= 0.4 ? 'bg-orange-500' : 'bg-red-500'
                                    }`}
                          ...
                          <TableCell>
                            {parseFloat(item.confidence || '0') >= 0.8 ? (
                              <Check className="w-4 h-4 text-green-500" />
                            ) : (
                              <AlertCircle className="w-4 h-4 text-yellow-500" />
                            )}
                  ...
                  {lineItemClassifications.length > 10 && (
                    <div className="mt-4 text-center text-sm text-gray-500">
                      Showing 10 of {lineItemClassifications.length} classifications
                    </div>
                  )}
                </CardContent>
              </Card>
